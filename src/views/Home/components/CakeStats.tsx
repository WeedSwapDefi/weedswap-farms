...
const CakeStats = () => {
  ...
  const weedswapPrice = usePriceCakeBusd()
  const circSupply = totalSupply ? totalSupply.minus(burnedBalance) : new BigNumber(0)
  const cakeSupply = getBalanceNumber(circSupply)
  const marketCap = weedswapPrice.times(circSupply)

  let weedswapPerBlock = 0
  if (farms && farms[0] && farms[0].weedswapPerBlock) {
    weedswapPerBlock = new BigNumber(farms[0].weedswapPerBlock).div(new BigNumber(10).pow(18)).toNumber()
  }
    ...
    <StyledCakeStats>
      <CardBody>
        <Heading size="lg" mb="24px">
          {TranslateString(534, 'WEED Stats')}
        </Heading>
        <RowHighlighted>
          <Text fontSize="14px">
            <strong>
        ...
        <Row>
          <Text fontSize="14px">
            {TranslateString(538, 'WEED smoked by WeedSwap')}{' '}
        ...
        <Row>
          <Text fontSize="14px">{TranslateString(540, 'New WEED/block')}</Text>
