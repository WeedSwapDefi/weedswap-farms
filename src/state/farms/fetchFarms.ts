...
const fetchFarms = async () => {
  const data = await Promise.all(
    farmsConfig.map(async (farmConfig) => {
      ...
      const [info, totalAllocPoint, poolInfo, weedswapPerBlock] = await multicall(masterchefABI, [
        {
      ...
      return {
        ...
        weedswapPerBlock: new BigNumber(weedswapPerBlock).toNumber(),
